...
<form name="frmlogin" action="{{ route('wap.dashubaologin') }}{{ !empty($redirect_url)  ? '?redirect_url='.$redirect_url : '' }}" method="post" id="frmlogin">
  {{ csrf_field() }}
  <div class="login">
    ...
    <a class="login_btn" href="{{ route('wap.dashubaoregister') }}{{ !empty($redirect_url)  ? '?redirect_url='.$redirect_url : '' }}">没有账号？点击注册</a>
	</div>
</form>
