...
@section('content')
<form name="frmregister" action="{{ route('wap.dashubaoregister') }}{{ !empty($redirect_url)  ? '?redirect_url='.$redirect_url : '' }}" method="post">
    {{ csrf_field() }}
    <div class="login">
      ...
      <a class="login_btn" href="{{ route('wap.dashubaologin') }}{{ !empty($redirect_url)  ? '?redirect_url='.$redirect_url : '' }}">已有账号？点击登录</a>
    </div>
  </form>
@endsection
